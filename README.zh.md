...
- LangChain
- OpenAI-compatible 聊天模型（通过 `langchain-openai`）
- Matplotlib
...
Qwen/DashScope 通过上面的 OpenAI-compatible 变量接入。要切换到其他
OpenAI-compatible 服务，只需要替换 `LLM_MODEL`、`OPENAI_COMPATIBLE_API_KEY`
和 `OPENAI_COMPATIBLE_BASE_URL`。
